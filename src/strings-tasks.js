...
function removeFirstOccurrences(str, value) {
  const startOfSubstr = str.indexOf(value);
  const endOfSubstr = startOfSubstr + value.length;

  if (startOfSubstr !== -1) {
    return `${str.slice(0, startOfSubstr) + str.slice(endOfSubstr)}`;
  }
...
function removeLastOccurrences(str, value) {
  const startOfSubstr = str.lastIndexOf(value);
  const endOfSubstr = startOfSubstr + value.length;
...
function isPalindrome(str) {
  return (
    str.toUpperCase().replaceAll(/\W/g, '') ===
    reverseString(str).toUpperCase().replaceAll(/\W/g, '')
  );
}
...
function reverseWords(str) {
  ...
  for (let i = 0; i < words.length; i += 1) {
    ...
    if ((words.length !== 1 && i === 0) || i !== words.length - 1) {
      sentence += ' ';
    }
  }
